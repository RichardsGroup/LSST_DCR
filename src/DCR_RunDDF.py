"""
Run DCR Precision Metric in the DDFs on all opsim using SciServer.
"""
import pandas as pd
import numpy as np
import os, sys
...
def run_sf_ddf(run, src_mags, dbDir, outDir, metricDataPath, **kwargs):
    ...
    try:
        ...
        summaryMetrics = [metrics.MedianMetric(), metrics.MeanMetric(), metrics.RmsMetric(),
                          ...
                          metrics.PercentileMetric(percentile=75, metricName='p75')]
        ...
        for band in src_mags:
            ...
            for mag in mags:
                ...
                for ddf in ddfFields:                
                    ...
                    DCR_metric = DCRPrecisionMetric(band, src_mag=mag)
                    DCR_metric.name = DCR_metric.metricName + f'_{ddf}'
                    mb = metricBundles.MetricBundle(DCR_metric, slicer, ddf_constraint)
                    bundleDict[DCR_metric.name] = mb
